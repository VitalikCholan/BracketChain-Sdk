...
- [What's in the box](#whats-in-the-box) — protocol-only client, determinism artifacts, 27 errors, 5 PDA helpers
- [Quick start](#quick-start) — read-only, write, settlement flows, subscribe
- [Public surface](#public-surface) — every export, in tables
- [Architecture notes](#architecture-notes) — protocol-only rationale, MVP `subscribe`, IDL vendoring
...
The 0.4 → 0.6 line keeps client construction stable (one breaking removal: `BracketChainIndexerClient` left the package in 0.6.0 — see the migration note in [What's in the box](#whats-in-the-box)), behind the Phase 1 program redeploy at `EF19YVUerm5QW1CsZeqiPDAFFtaXgdt6WuYBGeiz9Q1z`:
