/**
 * This code was AUTOGENERATED using the Codama library.
 * Please DO NOT EDIT THIS FILE, instead use visitors
 * to add features, then rerun Codama to update it.
 *
 * @see https://github.com/codama-idl/codama
 */

import {
  combineCodec,
  fixDecoderSize,
  fixEncoderSize,
  getBytesDecoder,
  getBytesEncoder,
  getStructDecoder,
  getStructEncoder,
  SOLANA_ERROR__PROGRAM_CLIENTS__INSUFFICIENT_ACCOUNT_METAS,
  SolanaError,
  transformEncoder,
  type AccountMeta,
  type AccountSignerMeta,
  type Address,
  type FixedSizeCodec,
  type FixedSizeDecoder,
  type FixedSizeEncoder,
  type Instruction,
  type InstructionWithAccounts,
  type InstructionWithData,
  type ReadonlyAccount,
  type ReadonlySignerAccount,
  type ReadonlyUint8Array,
  type TransactionSigner,
  type WritableAccount,
} from "@solana/kit";
import {
  getAccountMetaFactory,
  type ResolvedInstructionAccount,
} from "@solana/program-client-core";
import { findProtocolConfigPda } from "../pdas";
import { BRACKET_CHAIN_PROGRAM_ADDRESS } from "../programs";

export const PROPOSE_RESULT_ORACLE_DISCRIMINATOR: ReadonlyUint8Array =
  new Uint8Array([252, 97, 195, 178, 97, 244, 118, 88]);

export function getProposeResultOracleDiscriminatorBytes(): ReadonlyUint8Array {
  return fixEncoderSize(getBytesEncoder(), 8).encode(
    PROPOSE_RESULT_ORACLE_DISCRIMINATOR,
  );
}

export type ProposeResultOracleInstruction<
  TProgram extends string = typeof BRACKET_CHAIN_PROGRAM_ADDRESS,
  TAccountRelayer extends string | AccountMeta<string> = string,
  TAccountTournament extends string | AccountMeta<string> = string,
  TAccountMatchAccount extends string | AccountMeta<string> = string,
  TAccountProtocolConfig extends string | AccountMeta<string> = string,
  TAccountSwitchboardFeed extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountRelayer extends string
        ? ReadonlySignerAccount<TAccountRelayer> &
            AccountSignerMeta<TAccountRelayer>
        : TAccountRelayer,
      TAccountTournament extends string
        ? ReadonlyAccount<TAccountTournament>
        : TAccountTournament,
      TAccountMatchAccount extends string
        ? WritableAccount<TAccountMatchAccount>
        : TAccountMatchAccount,
      TAccountProtocolConfig extends string
        ? ReadonlyAccount<TAccountProtocolConfig>
        : TAccountProtocolConfig,
      TAccountSwitchboardFeed extends string
        ? ReadonlyAccount<TAccountSwitchboardFeed>
        : TAccountSwitchboardFeed,
      ...TRemainingAccounts,
    ]
  >;

export type ProposeResultOracleInstructionData = {
  discriminator: ReadonlyUint8Array;
};

export type ProposeResultOracleInstructionDataArgs = {};

export function getProposeResultOracleInstructionDataEncoder(): FixedSizeEncoder<ProposeResultOracleInstructionDataArgs> {
  return transformEncoder(
    getStructEncoder([["discriminator", fixEncoderSize(getBytesEncoder(), 8)]]),
    (value) => ({
      ...value,
      discriminator: PROPOSE_RESULT_ORACLE_DISCRIMINATOR,
    }),
  );
}

export function getProposeResultOracleInstructionDataDecoder(): FixedSizeDecoder<ProposeResultOracleInstructionData> {
  return getStructDecoder([
    ["discriminator", fixDecoderSize(getBytesDecoder(), 8)],
  ]);
}

export function getProposeResultOracleInstructionDataCodec(): FixedSizeCodec<
  ProposeResultOracleInstructionDataArgs,
  ProposeResultOracleInstructionData
> {
  return combineCodec(
    getProposeResultOracleInstructionDataEncoder(),
    getProposeResultOracleInstructionDataDecoder(),
  );
}

export type ProposeResultOracleAsyncInput<
  TAccountRelayer extends string = string,
  TAccountTournament extends string = string,
  TAccountMatchAccount extends string = string,
  TAccountProtocolConfig extends string = string,
  TAccountSwitchboardFeed extends string = string,
> = {
  /**
   * Permissionless relayer / fee-payer — anyone may push the proposal; trust
   * bottoms out in the feed account contents, not the signer.
   */
  relayer: TransactionSigner<TAccountRelayer>;
  tournament: Address<TAccountTournament>;
  matchAccount: Address<TAccountMatchAccount>;
  protocolConfig?: Address<TAccountProtocolConfig>;
  switchboardFeed: Address<TAccountSwitchboardFeed>;
};

export async function getProposeResultOracleInstructionAsync<
  TAccountRelayer extends string,
  TAccountTournament extends string,
  TAccountMatchAccount extends string,
  TAccountProtocolConfig extends string,
  TAccountSwitchboardFeed extends string,
  TProgramAddress extends Address = typeof BRACKET_CHAIN_PROGRAM_ADDRESS,
>(
  input: ProposeResultOracleAsyncInput<
    TAccountRelayer,
    TAccountTournament,
    TAccountMatchAccount,
    TAccountProtocolConfig,
    TAccountSwitchboardFeed
  >,
  config?: { programAddress?: TProgramAddress },
): Promise<
  ProposeResultOracleInstruction<
    TProgramAddress,
    TAccountRelayer,
    TAccountTournament,
    TAccountMatchAccount,
    TAccountProtocolConfig,
    TAccountSwitchboardFeed
  >
> {
  // Program address.
  const programAddress =
    config?.programAddress ?? BRACKET_CHAIN_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    relayer: { value: input.relayer ?? null, isWritable: false },
    tournament: { value: input.tournament ?? null, isWritable: false },
    matchAccount: { value: input.matchAccount ?? null, isWritable: true },
    protocolConfig: { value: input.protocolConfig ?? null, isWritable: false },
    switchboardFeed: {
      value: input.switchboardFeed ?? null,
      isWritable: false,
    },
  };
  const accounts = originalAccounts as Record<
    keyof typeof originalAccounts,
    ResolvedInstructionAccount
  >;

  // Resolve default values.
  if (!accounts.protocolConfig.value) {
    accounts.protocolConfig.value = await findProtocolConfigPda();
  }

  const getAccountMeta = getAccountMetaFactory(programAddress, "programId");
  return Object.freeze({
    accounts: [
      getAccountMeta("relayer", accounts.relayer),
      getAccountMeta("tournament", accounts.tournament),
      getAccountMeta("matchAccount", accounts.matchAccount),
      getAccountMeta("protocolConfig", accounts.protocolConfig),
      getAccountMeta("switchboardFeed", accounts.switchboardFeed),
    ],
    data: getProposeResultOracleInstructionDataEncoder().encode({}),
    programAddress,
  } as ProposeResultOracleInstruction<
    TProgramAddress,
    TAccountRelayer,
    TAccountTournament,
    TAccountMatchAccount,
    TAccountProtocolConfig,
    TAccountSwitchboardFeed
  >);
}

export type ProposeResultOracleInput<
  TAccountRelayer extends string = string,
  TAccountTournament extends string = string,
  TAccountMatchAccount extends string = string,
  TAccountProtocolConfig extends string = string,
  TAccountSwitchboardFeed extends string = string,
> = {
  /**
   * Permissionless relayer / fee-payer — anyone may push the proposal; trust
   * bottoms out in the feed account contents, not the signer.
   */
  relayer: TransactionSigner<TAccountRelayer>;
  tournament: Address<TAccountTournament>;
  matchAccount: Address<TAccountMatchAccount>;
  protocolConfig: Address<TAccountProtocolConfig>;
  switchboardFeed: Address<TAccountSwitchboardFeed>;
};

export function getProposeResultOracleInstruction<
  TAccountRelayer extends string,
  TAccountTournament extends string,
  TAccountMatchAccount extends string,
  TAccountProtocolConfig extends string,
  TAccountSwitchboardFeed extends string,
  TProgramAddress extends Address = typeof BRACKET_CHAIN_PROGRAM_ADDRESS,
>(
  input: ProposeResultOracleInput<
    TAccountRelayer,
    TAccountTournament,
    TAccountMatchAccount,
    TAccountProtocolConfig,
    TAccountSwitchboardFeed
  >,
  config?: { programAddress?: TProgramAddress },
): ProposeResultOracleInstruction<
  TProgramAddress,
  TAccountRelayer,
  TAccountTournament,
  TAccountMatchAccount,
  TAccountProtocolConfig,
  TAccountSwitchboardFeed
> {
  // Program address.
  const programAddress =
    config?.programAddress ?? BRACKET_CHAIN_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    relayer: { value: input.relayer ?? null, isWritable: false },
    tournament: { value: input.tournament ?? null, isWritable: false },
    matchAccount: { value: input.matchAccount ?? null, isWritable: true },
    protocolConfig: { value: input.protocolConfig ?? null, isWritable: false },
    switchboardFeed: {
      value: input.switchboardFeed ?? null,
      isWritable: false,
    },
  };
  const accounts = originalAccounts as Record<
    keyof typeof originalAccounts,
    ResolvedInstructionAccount
  >;

  const getAccountMeta = getAccountMetaFactory(programAddress, "programId");
  return Object.freeze({
    accounts: [
      getAccountMeta("relayer", accounts.relayer),
      getAccountMeta("tournament", accounts.tournament),
      getAccountMeta("matchAccount", accounts.matchAccount),
      getAccountMeta("protocolConfig", accounts.protocolConfig),
      getAccountMeta("switchboardFeed", accounts.switchboardFeed),
    ],
    data: getProposeResultOracleInstructionDataEncoder().encode({}),
    programAddress,
  } as ProposeResultOracleInstruction<
    TProgramAddress,
    TAccountRelayer,
    TAccountTournament,
    TAccountMatchAccount,
    TAccountProtocolConfig,
    TAccountSwitchboardFeed
  >);
}

export type ParsedProposeResultOracleInstruction<
  TProgram extends string = typeof BRACKET_CHAIN_PROGRAM_ADDRESS,
  TAccountMetas extends readonly AccountMeta[] = readonly AccountMeta[],
> = {
  programAddress: Address<TProgram>;
  accounts: {
    /**
     * Permissionless relayer / fee-payer — anyone may push the proposal; trust
     * bottoms out in the feed account contents, not the signer.
     */
    relayer: TAccountMetas[0];
    tournament: TAccountMetas[1];
    matchAccount: TAccountMetas[2];
    protocolConfig: TAccountMetas[3];
    switchboardFeed: TAccountMetas[4];
  };
  data: ProposeResultOracleInstructionData;
};

export function parseProposeResultOracleInstruction<
  TProgram extends string,
  TAccountMetas extends readonly AccountMeta[],
>(
  instruction: Instruction<TProgram> &
    InstructionWithAccounts<TAccountMetas> &
    InstructionWithData<ReadonlyUint8Array>,
): ParsedProposeResultOracleInstruction<TProgram, TAccountMetas> {
  if (instruction.accounts.length < 5) {
    throw new SolanaError(
      SOLANA_ERROR__PROGRAM_CLIENTS__INSUFFICIENT_ACCOUNT_METAS,
      {
        actualAccountMetas: instruction.accounts.length,
        expectedAccountMetas: 5,
      },
    );
  }
  let accountIndex = 0;
  const getNextAccount = () => {
    const accountMeta = (instruction.accounts as TAccountMetas)[accountIndex]!;
    accountIndex += 1;
    return accountMeta;
  };
  return {
    programAddress: instruction.programAddress,
    accounts: {
      relayer: getNextAccount(),
      tournament: getNextAccount(),
      matchAccount: getNextAccount(),
      protocolConfig: getNextAccount(),
      switchboardFeed: getNextAccount(),
    },
    data: getProposeResultOracleInstructionDataDecoder().decode(
      instruction.data,
    ),
  };
}
